...
HTTP 地址的独立窗口/安装行为取决于系统。独立窗口可能不继承浏览器认证，可用二维码图片重新配对。HTTP 不支持网页内实时摄像头扫码或后台推送。Service Worker 仅处理通知，不缓存私有对话，也不离线执行或后台重发指令。

## 任务完成通知

通过 HTTPS 打开 Pokite；iPhone/iPad 需要先添加到主屏幕并从那里启动。
选择项目，点击侧栏的铃铛，开启通知并关注当前项目。此后电脑端或手机端发起的
任务完成、失败时，Pokite 会尝试推送，点击可进入对应会话。其他项目需分别关注。

通知默认不包含项目名称和对话正文。Mac 与 Pokite 需保持运行并连接互联网；
通知通过浏览器厂商的推送服务发送，无需 Pokite 云中转或公网入站端口。
点击后访问电脑仍需局域网或 Tailscale 连通。可发送测试通知、取消项目关注或
关闭本设备通知；系统专注模式、网络和推送服务可能影响送达。推送服务接收
不等于手机已显示。当前通过轮询检测状态，未观察到明确完成证据的任务不会
猜测为成功；断网期间短暂发生且未保留的原生状态可能无法恢复。
