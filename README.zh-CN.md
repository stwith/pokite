...
点击侧栏的铃铛，开启本设备通知。所有已接入 Agent 的项目均纳入监听，
不需要逐项目关注。任务完成、失败时会尝试推送，点击可进入对应会话。
新项目自动发现，历史完成记录不补发。

通知显示 Agent、任务状态、会话标题和项目路径，不包含对话正文。Mac 与 Pokite 需保持运行并连接互联网；
通知通过浏览器厂商的推送服务发送，无需 Pokite 云中转或公网入站端口。
点击后访问电脑仍需局域网或 Tailscale 连通。可单独关闭本设备通知；
系统专注模式、网络和推送服务可能影响送达。推送服务接收
